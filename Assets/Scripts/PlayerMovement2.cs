...
{   
    ...
    moveInputSideways = Input.GetAxisRaw("Horizontal2");
...
if(animator.GetCurrentAnimatorClipInfo(0)[0].clip.name != "Player_death")
    {
        ...
        {
            if(boostAnimationDone == true && GetComponent<PlayerCombat2>().attacking == false)
            {   
    ...
    if(boostUsed == false)
        ...
        if(speedBoost == true)
            ...
            else
            {
                boostTime = boostTimeStart; 
                player.velocity = new Vector2(moveInputSideways * speed * Time.deltaTime , player.velocity.y);     
                GetComponent<BushHitPlayer2>().enabled = true;
    ...
    if(Physics2D.OverlapCircle(groundCheck.position, 0.05f, backgroundMask) == true && jumpKey == true)
     {  
        doubleJump = 0;
        jumpKey = false;
